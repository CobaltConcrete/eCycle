...
const Map = () => {
    ...
    useEffect(() => {
        const fetchHistory = async () => {
            try {
                const response = await axios.get(`http://${process.env.REACT_APP_localhost}:5000/get-history`, {
                    params: { userid }
                });
    ...
    const addMarkersToMap = (locations) => {
        ...
        locations.forEach((location) => {
            ...
            marker.addListener('click', async () => {
                ...
                try {
                    const response = await axios.post(`http://${process.env.REACT_APP_localhost}:5000/add-history`, {
